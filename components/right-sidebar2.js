...
class RightSideBar2 extends HTMLElement {
	constructor() {
		super();
		// Attach shadow root
	...
	async connectedCallback() {
		...
		// Fetch HTML
		const htmlResponse = await fetch('../views/right-sidebar2.html');
		const htmlContent = await htmlResponse.text();
	
		// Fetch CSS
		const cssResponse = await fetch('../assets/css/right-sidebar2.css');
		const cssContent = await cssResponse.text();

		// Fetch JavaScript
		const jsResponse = await fetch('../scripts/right-sidebar2.js');
  ...
  customElements.define('right-sidebar2-component', RightSideBar2);
